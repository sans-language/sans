...
fn generate_llvm<'ctx>(
    ...
    let accept_type = i64_type.fn_type(&[i64_type.into(), i8_ptr_type.into(), i8_ptr_type.into()], false);
    llvm_module.add_function("accept", accept_type, Some(Linkage::External));

    let recv_type = i64_type.fn_type(&[i64_type.into(), i8_ptr_type.into(), i64_type.into(), i64_type.into()], false);
    llvm_module.add_function("recv", recv_type, Some(Linkage::External));

    let send_type = i64_type.fn_type(&[i64_type.into(), i8_ptr_type.into(), i64_type.into(), i64_type.into()], false);
    llvm_module.add_function("send", send_type, Some(Linkage::External));
    ...
    // SSL/TLS (OpenSSL)
    llvm_module.add_function("TLS_server_method", ptr_type.fn_type(&[], false), Some(Linkage::External));
    llvm_module.add_function("SSL_CTX_new", ptr_type.fn_type(&[ptr_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_CTX_use_certificate_file", i32_type.fn_type(&[ptr_type.into(), ptr_type.into(), i32_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_CTX_use_PrivateKey_file", i32_type.fn_type(&[ptr_type.into(), ptr_type.into(), i32_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_new", ptr_type.fn_type(&[ptr_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_set_fd", i32_type.fn_type(&[ptr_type.into(), i32_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_accept", i32_type.fn_type(&[ptr_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_read", i32_type.fn_type(&[ptr_type.into(), ptr_type.into(), i32_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_write", i32_type.fn_type(&[ptr_type.into(), ptr_type.into(), i32_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_shutdown", i32_type.fn_type(&[ptr_type.into()], false), Some(Linkage::External));
    llvm_module.add_function("SSL_free", context.void_type().fn_type(&[ptr_type.into()], false), Some(Linkage::External));
        ...
        for instr in &func.body {
            ...
            match instr {
                ...
                Instruction::FptrNamed { dest, func_name } => {
                    ...
                    let fn_ptr = fn_val.as_global_value().as_pointer_value();
                    let int_val = builder.build_ptr_to_int(fn_ptr, i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), int_val);
                    ptrs.insert(dest.clone(), fn_ptr);
                }
                Instruction::Fcall { dest, fn_ptr, arg } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let fn_type = i64_type.fn_type(&[i64_type.into()], false);
                    let fp = if let Some(p) = ptrs.get(fn_ptr) { *p } else {
                        builder.build_int_to_ptr(regs[fn_ptr], ptr_type, "fcall_fp").map_err(|e| CodegenError::LlvmError(e.to_string()))?
                    };
                    let call = builder.build_indirect_call(fn_type, fp, &[regs[arg].into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("fcall: expected return value".into())),
                    };
                    regs.insert(dest.clone(), result);
                }
                ...
                Instruction::Alloc { dest, size } => {
                    let size_val = regs[size];
                    let malloc_fn = llvm_module.get_function("malloc").unwrap();
                    let call = builder.build_call(malloc_fn, &[size_val.into()], "alloc_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let ptr_val = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_pointer_value(),
                        _ => return Err(CodegenError::LlvmError("malloc: expected return".into())),
                    };
                    let int_val = builder.build_ptr_to_int(ptr_val, i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), int_val);
                }
                Instruction::Dealloc { dest, ptr } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let ptr_val = builder.build_int_to_ptr(regs[ptr], ptr_type, "dealloc_p")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let free_fn = llvm_module.get_function("free").unwrap();
                    builder.build_call(free_fn, &[ptr_val.into()], "")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), i64_type.const_int(0, false));
                }
                Instruction::Ralloc { dest, ptr, size } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let ptr_val = builder.build_int_to_ptr(regs[ptr], ptr_type, "ralloc_p")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let size_val = regs[size];
                    let realloc_fn = llvm_module.get_function("realloc").unwrap();
                    let call = builder.build_call(realloc_fn, &[ptr_val.into(), size_val.into()], "ralloc_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let new_ptr = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_pointer_value(),
                        _ => return Err(CodegenError::LlvmError("realloc: expected return".into())),
                    };
                    let int_val = builder.build_ptr_to_int(new_ptr, i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), int_val);
                }
                Instruction::Mcpy { dest, dst_ptr, src_ptr, len } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let dp = builder.build_int_to_ptr(regs[dst_ptr], ptr_type, "mcpy_d")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let sp = builder.build_int_to_ptr(regs[src_ptr], ptr_type, "mcpy_s")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                ...
                Instruction::Mcmp { dest, a_ptr, b_ptr, len } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let ap = builder.build_int_to_ptr(regs[a_ptr], ptr_type, "mcmp_a")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let bp = builder.build_int_to_ptr(regs[b_ptr], ptr_type, "mcmp_b")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let len_val = regs[len];
                    let memcmp_fn = llvm_module.get_function("memcmp").unwrap();
                    let call = builder.build_call(memcmp_fn, &[ap.into(), bp.into(), len_val.into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("memcmp: expected return".into())),
                    };
                    let ext = builder.build_int_s_extend(result, i64_type, &format!("{}_ext", dest))
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), ext);
                }
                Instruction::Slen { dest, ptr } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let p = builder.build_int_to_ptr(regs[ptr], ptr_type, "slen_p")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let strlen_fn = llvm_module.get_function("strlen").unwrap();
                    let call = builder.build_call(strlen_fn, &[p.into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("strlen: expected return".into())),
                ...
                Instruction::Load8 { dest, ptr } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let p = builder.build_int_to_ptr(regs[ptr], ptr_type, "ld8_p")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let i8_type = context.i8_type();
                    let byte = builder.build_load(i8_type, p, "ld8_v")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?.into_int_value();
                    let ext = builder.build_int_z_extend(byte, i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), ext);
                }
                Instruction::Store8 { dest, ptr, val } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let p = builder.build_int_to_ptr(regs[ptr], ptr_type, "st8_p")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let i8_type = context.i8_type();
                ...
                Instruction::HttpBody { dest, response } => {
                    ...
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let ptr_val = builder.build_int_to_ptr(as_int, ptr_type, "hbody_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), as_int);
                    ptrs.insert(dest.clone(), ptr_val);
                }
                ...
                // Socket primitives
                Instruction::Sock { dest, domain, sock_type, proto } => {
                    let fn_ref = llvm_module.get_function("socket").unwrap();
                    let call = builder.build_call(fn_ref, &[regs[domain].into(), regs[sock_type].into(), regs[proto].into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("socket: expected return".into())),
                    };
                    regs.insert(dest.clone(), result);
                }
                ...
                Instruction::Slisten { dest, fd, backlog } => {
                    let fn_ref = llvm_module.get_function("listen").unwrap();
                    let call = builder.build_call(fn_ref, &[regs[fd].into(), regs[backlog].into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("listen: expected return".into())),
                    };
                    regs.insert(dest.clone(), result);
                }
                Instruction::Saccept { dest, fd } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let null = ptr_type.const_null();
                    let fn_ref = llvm_module.get_function("accept").unwrap();
                    let call = builder.build_call(fn_ref, &[regs[fd].into(), null.into(), null.into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("accept: expected return".into())),
                    };
                    regs.insert(dest.clone(), result);
                }
                Instruction::Srecv { dest, fd, buf, len } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let buf_ptr = builder.build_int_to_ptr(regs[buf], ptr_type, "recv_bp")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let fn_ref = llvm_module.get_function("recv").unwrap();
                    let zero = i64_type.const_int(0, false);
                    let call = builder.build_call(fn_ref, &[regs[fd].into(), buf_ptr.into(), regs[len].into(), zero.into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("recv: expected return".into())),
                    };
                    regs.insert(dest.clone(), result);
                }
                Instruction::Ssend { dest, fd, buf, len } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let buf_ptr = builder.build_int_to_ptr(regs[buf], ptr_type, "send_bp")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let fn_ref = llvm_module.get_function("send").unwrap();
                    let zero = i64_type.const_int(0, false);
                    let call = builder.build_call(fn_ref, &[regs[fd].into(), buf_ptr.into(), regs[len].into(), zero.into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("send: expected return".into())),
                    };
                    regs.insert(dest.clone(), result);
                }
                Instruction::Sclose { dest, fd } => {
                    let fn_ref = llvm_module.get_function("close").unwrap();
                    let call = builder.build_call(fn_ref, &[regs[fd].into()], dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("close: expected return".into())),
                    };
                    regs.insert(dest.clone(), result);
                }
                Instruction::SslCtx { dest, cert, key } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let cert_ptr = builder.build_int_to_ptr(regs[cert], ptr_type, "ssl_cert_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let key_ptr = builder.build_int_to_ptr(regs[key], ptr_type, "ssl_key_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    // TLS_server_method()
                    let method_fn = llvm_module.get_function("TLS_server_method").unwrap();
                    let method_call = builder.build_call(method_fn, &[], "ssl_method")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let method_ptr = match method_call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_pointer_value(),
                        _ => return Err(CodegenError::LlvmError("TLS_server_method: expected return".into())),
                    };
                    // SSL_CTX_new(method)
                    let ctx_fn = llvm_module.get_function("SSL_CTX_new").unwrap();
                    let ctx_call = builder.build_call(ctx_fn, &[method_ptr.into()], "ssl_ctx")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let ctx_ptr = match ctx_call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_pointer_value(),
                        _ => return Err(CodegenError::LlvmError("SSL_CTX_new: expected return".into())),
                    };
                    // SSL_CTX_use_certificate_file(ctx, cert, 1=PEM)
                    let i32_type = context.i32_type();
                    let one_i32 = i32_type.const_int(1, false);
                    let cert_fn = llvm_module.get_function("SSL_CTX_use_certificate_file").unwrap();
                    builder.build_call(cert_fn, &[ctx_ptr.into(), cert_ptr.into(), one_i32.into()], "ssl_cert_r")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    // SSL_CTX_use_PrivateKey_file(ctx, key, 1=PEM)
                    let key_fn = llvm_module.get_function("SSL_CTX_use_PrivateKey_file").unwrap();
                    builder.build_call(key_fn, &[ctx_ptr.into(), key_ptr.into(), one_i32.into()], "ssl_key_r")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    // ptrtoint ctx_ptr -> i64
                    let result = builder.build_ptr_to_int(ctx_ptr, i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), result);
                }
                Instruction::SslAccept { dest, ctx, fd } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let i32_type = context.i32_type();
                    let ctx_ptr = builder.build_int_to_ptr(regs[ctx], ptr_type, "ssl_ctx_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    // SSL_new(ctx)
                    let ssl_new_fn = llvm_module.get_function("SSL_new").unwrap();
                    let ssl_call = builder.build_call(ssl_new_fn, &[ctx_ptr.into()], "ssl_new")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let ssl_ptr = match ssl_call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_pointer_value(),
                        _ => return Err(CodegenError::LlvmError("SSL_new: expected return".into())),
                    };
                    // SSL_set_fd(ssl, fd_i32)
                    let fd_i32 = builder.build_int_truncate(regs[fd], i32_type, "ssl_fd_i32")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let set_fd_fn = llvm_module.get_function("SSL_set_fd").unwrap();
                    builder.build_call(set_fd_fn, &[ssl_ptr.into(), fd_i32.into()], "ssl_setfd_r")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    // SSL_accept(ssl)
                    let accept_fn = llvm_module.get_function("SSL_accept").unwrap();
                    let accept_call = builder.build_call(accept_fn, &[ssl_ptr.into()], "ssl_accept_r")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let accept_result = match accept_call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("SSL_accept: expected return".into())),
                    };
                    // Branch: if result <= 0, return 0; else return ptrtoint ssl_ptr
                    let zero_i32 = i32_type.const_int(0, false);
                    let cmp = builder.build_int_compare(inkwell::IntPredicate::SGT, accept_result, zero_i32, "ssl_accept_ok")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let current_fn = builder.get_insert_block().unwrap().get_parent().unwrap();
                    let then_bb = context.append_basic_block(current_fn, "ssl_accept_ok");
                    let else_bb = context.append_basic_block(current_fn, "ssl_accept_fail");
                    let merge_bb = context.append_basic_block(current_fn, "ssl_accept_merge");
                    builder.build_conditional_branch(cmp, then_bb, else_bb)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    // then: ptrtoint ssl_ptr
                    builder.position_at_end(then_bb);
                    let ssl_int = builder.build_ptr_to_int(ssl_ptr, i64_type, "ssl_ptr_int")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    builder.build_unconditional_branch(merge_bb)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let then_end = builder.get_insert_block().unwrap();
                    ...
                    let phi = builder.build_phi(i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    phi.add_incoming(&[(&ssl_int, then_end), (&zero_val, else_end)]);
                    let val = phi.as_basic_value().into_int_value();
                    regs.insert(dest.clone(), val);
                }
                Instruction::SslRead { dest, ssl, buf, len } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let i32_type = context.i32_type();
                    let ssl_ptr = builder.build_int_to_ptr(regs[ssl], ptr_type, "sslr_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let buf_ptr = builder.build_int_to_ptr(regs[buf], ptr_type, "sslr_buf")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let len_i32 = builder.build_int_truncate(regs[len], i32_type, "sslr_len")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let fn_ref = llvm_module.get_function("SSL_read").unwrap();
                    let call = builder.build_call(fn_ref, &[ssl_ptr.into(), buf_ptr.into(), len_i32.into()], "sslr_r")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result_i32 = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("SSL_read: expected return".into())),
                    };
                    let result = builder.build_int_s_extend(result_i32, i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), result);
                }
                Instruction::SslWrite { dest, ssl, buf, len } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let i32_type = context.i32_type();
                    let ssl_ptr = builder.build_int_to_ptr(regs[ssl], ptr_type, "sslw_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let buf_ptr = builder.build_int_to_ptr(regs[buf], ptr_type, "sslw_buf")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let len_i32 = builder.build_int_truncate(regs[len], i32_type, "sslw_len")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let fn_ref = llvm_module.get_function("SSL_write").unwrap();
                    let call = builder.build_call(fn_ref, &[ssl_ptr.into(), buf_ptr.into(), len_i32.into()], "sslw_r")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result_i32 = match call.try_as_basic_value() {
                        inkwell::values::ValueKind::Basic(bv) => bv.into_int_value(),
                        _ => return Err(CodegenError::LlvmError("SSL_write: expected return".into())),
                    };
                    let result = builder.build_int_s_extend(result_i32, i64_type, dest)
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    regs.insert(dest.clone(), result);
                }
                Instruction::SslClose { dest, ssl } => {
                    let ptr_type = context.ptr_type(inkwell::AddressSpace::default());
                    let ssl_ptr = builder.build_int_to_ptr(regs[ssl], ptr_type, "sslc_ptr")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let shutdown_fn = llvm_module.get_function("SSL_shutdown").unwrap();
                    builder.build_call(shutdown_fn, &[ssl_ptr.into()], "sslc_shut")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let free_fn = llvm_module.get_function("SSL_free").unwrap();
                    builder.build_call(free_fn, &[ssl_ptr.into()], "")
                        .map_err(|e| CodegenError::LlvmError(e.to_string()))?;
                    let result = i64_type.const_int(0, false);
                    regs.insert(dest.clone(), result);
                }
