...
| `crates/cyflym-typeck/src/types.rs` | Modify | Add `JsonValue` variant to `Type` enum + Display |
| `crates/cyflym-typeck/src/lib.rs` | Modify | Type-check 8 built-in functions + 9 methods on JsonValue |
| `crates/cyflym-ir/src/ir.rs` | Modify | Add `JsonValue` to IrType, add ~17 new Instruction variants |
| `crates/cyflym-ir/src/lib.rs` | Modify | Add `ir_type_for_return` case, lower built-in calls + methods |
| `crates/cyflym-codegen/src/lib.rs` | Modify | Declare 17 extern C functions, compile JSON instructions |
| `crates/cyflym-driver/src/main.rs` | Modify | Compile json.c + link json.o alongside user code |
| `crates/cyflym-driver/tests/e2e.rs` | Modify | Update both helpers to compile+link json.c, add 5 E2E tests |
| `tests/fixtures/json_build.cy` | Create | E2E: build object, stringify, exit with length |
| `tests/fixtures/json_parse_access.cy` | Create | E2E: parse JSON, access fields, exit with sum |
| `tests/fixtures/json_roundtrip.cy` | Create | E2E: build → stringify → parse → verify |
| `tests/fixtures/json_object_stringify.cy` | Create | E2E: empty object stringify |
| `tests/fixtures/json_int_roundtrip.cy` | Create | E2E: json_int → get_int |
...
**Files:**
- Modify: `crates/cyflym-typeck/src/types.rs` (lines 1-39)
- Modify: `crates/cyflym-typeck/src/lib.rs` (built-in checks ~lines 616-663, method checks ~lines 1047+)
- Test: `crates/cyflym-typeck/src/lib.rs` (test module at end of file)

- [ ] **Step 1: Add `JsonValue` to Type enum and Display**

In `crates/cyflym-typeck/src/types.rs`, add `JsonValue` as a new variant to the `Type` enum (after `Array`):
...
Add these tests at the end of the test module in `crates/cyflym-typeck/src/lib.rs`:
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym-typeck check_json 2>&1 | tail -20`
Expected: FAIL — "undefined variable" errors for json_parse etc.

- [ ] **Step 4: Implement JSON built-in function type checking**

In `crates/cyflym-typeck/src/lib.rs`, in the `Expr::Call` arm of `check_expr`, after the `file_exists` block (~line 665), add. **Important:** Follow the existing codebase patterns exactly — `check_expr` is a free function (not a method), takes 10 arguments, and `TypeError::new` takes a single message argument (string or format!).
...
Add to test module in `crates/cyflym-typeck/src/lib.rs`:
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym-typeck check_json 2>&1 | tail -30`
Expected: New method tests FAIL

- [ ] **Step 8: Implement JsonValue method type checking**

In `crates/cyflym-typeck/src/lib.rs`, in the `Expr::MethodCall` arm of `check_expr`, find the `match (&obj_ty, method.as_str())` block (~line 1004). The existing match has arms for Sender, Receiver, JoinHandle, Mutex, Array, and String types. Add `JsonValue` arms before the `_ => {}` catch-all (~line 1111). **Important:** Use the same tuple pattern style `(Type::JsonValue, "method") =>` and the same `check_expr` free function call with all 10 arguments.
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym-typeck 2>&1 | tail -5`
...
**Files:**
- Modify: `crates/cyflym-ir/src/ir.rs` (IrType enum ~line 9, Instruction enum ~lines 17-153)
- Modify: `crates/cyflym-ir/src/lib.rs` (ir_type_for_return ~lines 11-22, lowering ~lines 431-491 for calls, ~lines 647+ for methods)
- Test: `crates/cyflym-ir/src/lib.rs` (test module at end of file)

- [ ] **Step 1: Add `JsonValue` to IrType enum**

In `crates/cyflym-ir/src/ir.rs`, add `JsonValue` to the IrType enum (~line 9):
...
In `crates/cyflym-ir/src/ir.rs`, add after the FileExists variant (~line 152):
...
In `crates/cyflym-ir/src/lib.rs`, in the `ir_type_for_return` function (~line 11-22), add before the `_ =>` fallback:
...
In `crates/cyflym-ir/src/lib.rs`, in the `print` handler (~line 416-423), the match on `ty` is exhaustive over all IrType variants. Add `IrType::JsonValue` alongside the other non-printable types:
...
Add to test module in `crates/cyflym-ir/src/lib.rs`:
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym-ir lower_json 2>&1 | tail -20`
Expected: FAIL — compilation errors or missing instruction variants

- [ ] **Step 6: Implement JSON built-in function lowering**

In `crates/cyflym-ir/src/lib.rs`, in the `Expr::Call` arm of `lower_expr`, after the file I/O cases (~line 491), add:
...
In `crates/cyflym-ir/src/lib.rs`, in the `Expr::MethodCall` arm of `lower_expr`, find where built-in type methods are matched (~line 647+). Add a block for `IrType::JsonValue` methods:
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym-ir 2>&1 | tail -5`
...
**Files:**
- Modify: `crates/cyflym-codegen/src/lib.rs` (extern declarations ~lines 100-145, instruction compilation)

- [ ] **Step 1: Declare all JSON C runtime external functions**

In `crates/cyflym-codegen/src/lib.rs`, after the `access` declaration (~line 145), add declarations for all 17 JSON C functions. Follow the existing pattern of declaring `FunctionType` and using `module.add_function`:
...
In the instruction match block of `crates/cyflym-codegen/src/lib.rs`, after the FileExists handler, add handlers for all JSON instructions. The pattern is consistent: get pointer from ptrs/regs, call C function, store result.
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo build -p cyflym-codegen 2>&1 | tail -10`
Expected: Compiles successfully

- [ ] **Step 4: Run all existing codegen tests (should still pass)**

Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym-codegen 2>&1 | tail -5`
...
**Files:**
- Modify: `crates/cyflym-driver/src/main.rs` (~lines 108-118)
- Modify: `crates/cyflym-driver/tests/e2e.rs` (~lines 69-73 and ~lines 112-116)

- [ ] **Step 1: Update `main.rs` to compile and link `json.c`**

In `crates/cyflym-driver/src/main.rs`, find the linking section (~line 108). Before the `cc` link command, add compilation of `json.c`. The runtime directory is at the workspace root:
...
In `crates/cyflym-driver/tests/e2e.rs`, in the `compile_and_run` function, before the link step (~line 112), add json.c compilation:
...
```rust
    ...
    let json_o_path = tmp_dir.join("cyflym_json_runtime_dir.o");
    let json_compile = Command::new("cc")
        .args(["-c", &json_c_path, "-o", json_o_path.to_str().unwrap()])
        .status()
        .expect("failed to compile json runtime");
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym --test e2e 2>&1 | tail -10`
Expected: All 24 existing E2E tests still pass

- [ ] **Step 5: Commit**

```bash
git add crates/cyflym-driver/src/main.rs crates/cyflym-driver/tests/e2e.rs
...
**Files:**
- Create: `tests/fixtures/json_object_stringify.cy`
- Create: `tests/fixtures/json_int_roundtrip.cy`
- Create: `tests/fixtures/json_build.cy`
- Create: `tests/fixtures/json_parse_access.cy`
- Create: `tests/fixtures/json_roundtrip.cy`
- Modify: `crates/cyflym-driver/tests/e2e.rs`

- [ ] **Step 1: Create `json_object_stringify.cy` fixture**
...
- [ ] **Step 2: Create `json_int_roundtrip.cy` fixture**
...
- [ ] **Step 3: Create `json_build.cy` fixture**
...
Exit code: length of `{"name":"cyflym","version":1,"tags":["fast","safe"]}` = 52.

- [ ] **Step 4: Create `json_parse_access.cy` fixture**
...
- [ ] **Step 5: Create `json_roundtrip.cy` fixture**
...
Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test -p cyflym --test e2e 2>&1 | tail -15`
Expected: All 29 E2E tests pass (24 existing + 5 new)

- [ ] **Step 8: Run the full test suite**

Run: `cd /Users/sgordon/Development/Repos/cyflym && LLVM_SYS_170_PREFIX=$(brew --prefix llvm@17) cargo test 2>&1 | tail -10`
Expected: All ~272 tests pass

- [ ] **Step 9: Commit**

```bash
git add tests/fixtures/json_object_stringify.cy tests/fixtures/json_int_roundtrip.cy tests/fixtures/json_build.cy tests/fixtures/json_parse_access.cy tests/fixtures/json_roundtrip.cy crates/cyflym-driver/tests/e2e.rs
